...
UNITTEST_START
{
  ...
  for(i = 0; list1[i].in; i++) {
    ...
    out = curl_json_unescape(list1[i].in, list1[i].inlen, &outlen);
    ...
    printf("curl_json_unescape test %d DONE\n", i);
  ...
  for(i = 0; list2[i].in; i++) {
    ...
    out = curl_easy_escape(list2[i].in, list2[i].inlen);
    abort_unless(out != NULL, "returned NULL!");
    ...
    printf("curl_json_escape test %d DONE (%s)\n", i, out);
